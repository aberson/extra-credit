...
[`docs/current-md-race-fix-plan.md`](../../docs/current-md-race-fix-plan.md)) are keyed by the
...
**Verified identical (2026-07-14):** the scratchpad-dir UUID
`cc4dfe4a-425f-4e2c-9fac-7b1f13164941` matched exactly the session transcript
`~/.claude/projects/c--Users-abero-dev/cc4dfe4a-425f-4e2c-9fac-7b1f13164941.jsonl`. Claude Code
...
`.claude/hooks/lib/task-state-derive.ps1` (`Write-DerivedRollup`). It is a pure function of the
...
- **Content contract** is owned by `.claude/skills/session-wrap/SKILL.md`:
...
- **Resume paths:** after `/clear`, the SessionStart hook injects `current.md`'s
  ...
  hook additionally injects a pointer to it (hook wiring lands with Step 5 / issue #295 —
  until then the pointer injection is spec, not live behavior). A fresh window (plain
...
The `SessionStart` hook (`session-resume.ps1`, matcher `compact|resume|clear` in
`.claude/settings.json`) is the primary path; it no-ops only on **no git root** or **no
