...
Steps 1-8 are merged and issues #1-#8 are closed: the runnable application foundation and its CI/browser gates, the secure profile-config round trip, the parent profile setup flow, all four v1 worksheet families (`dry-math`, `find-the-wow`, `sentence-builder`, `count-compare-make`) with their preview, print, and answer-key surfaces, and the reviewed decorative line-art system whose `frontend/src/web/assets/line-art/manifest.json` is mirrored by `ASSET_PROVENANCE.md` and governed by `CONTRIBUTING.md`. Local gates on `main` are 403 Vitest tests across 16 files, 37 Playwright specs, `eslint . --max-warnings 0` clean, and all three `tsc --noEmit` projects clean. Step 8 also added a mechanical guard: `frontend/tests/integration/plan-citations.test.ts` and its `plan-citations.json` manifest register every `plan.md:NNN` citation in source and fail on line drift, a wrong number, an unregistered site, a stale anchor, or a changed number inside an anchored line; `CONTRIBUTING.md` § "Comment claims" owns the rule. The independent nested repository is public at `https://github.com/aberson/extra-credit`; `plan.md` remains the canonical implementation plan, `documentation/extra-credit-proposal.html` is its confirmed revision-2 review surface, and the privacy-critical root `.gitignore` plus canonical MIT `LICENSE` are in place. Step 9, personalization and worksheet options (issue #9), which absorbs issues #14 and #16, has been built but is NOT merged: it lives on branch `build-step-1788124568` (commit `3597f7c`, 15 files / 2614 insertions) where its own gates are green at 422 Vitest across 18 files and 42 Playwright specs. It halted under the stop-and-audit rule on the third instance of one bug-shape — fixture sets that collapse the distinction the code discriminates on, leaving the discriminating arm unreachable while the suite stays green. Its behavior is correct and runtime-proven; what is missing is guard coverage. The audit, a 70-arm reachability map, and 16 reviewer reports are in that branch's `.build-step/`; the two live defects to close first are the ABSENT binding-maximum arms in `find-the-wow/definition.ts` and `dry-math/definition.ts` (their shortfall functions do not accept the skills, so no fixture can reach them). Open issues #15, #18, and #19 carry review findings that later steps absorb.
