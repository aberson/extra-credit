...
- Never automatically overwrite fatal-decode, malformed-JSON, schema-invalid, or future-version data. A fatal-decode/malformed/schema-invalid file may be replaced only through the explicit **Back up invalid file and replace** action with the raw-file ETag. Before replacement, write at most the already-bounded 64 KiB byte-for-byte to an exclusive sibling named `children.local.json.invalid-YYYYMMDDTHHMMSSZ-<8hex>.bak`, generating each `<8hex>` with `randomBytes(4).toString("hex")` and creating the candidate with `flag: "wx"`. Make at most eight candidate attempts and retry only an `EEXIST` collision. Any other creation error, or eight exhausted collisions, returns `CONFIG_IO_ERROR` and leaves the target untouched. Flush the successful backup, apply owner-only mode where supported, then replace the target. A future schema version is never eligible for this recovery action and requires a later migration or manual intervention.
...
Open `http://127.0.0.1:4311`. On first run, the setup screen creates the repository-root `config/children.local.json` after the parent saves a valid profile. Stop both development processes with `Ctrl+C` in the launching console.
...
**Next:** Steps 1–8 are merged and issues #1–#8 are closed. Step 9 (issue #9) is built but NOT merged: it sits on branch `build-step-1788124568` (commit `3597f7c`), halted at stop-and-audit with its own gates green (422 Vitest / 42 Playwright). Land the one structural coverage fix described in that branch's `.build-step/audit-stop-and-audit.md`, merge it, then resume `/build-phase --plan plan.md` at Step 10; the planning-to-build transition above is finished and must not be re-run.
...
- **Start-cmd:** `npm --prefix frontend run dev`
- **URL:** `http://127.0.0.1:4311`
- **Files:** `frontend/src/web/api/client.ts`, `frontend/src/web/profiles/ProfileList.tsx`, `frontend/src/web/profiles/ProfileEditor.tsx`, `frontend/src/web/profiles/MathSkillsEditor.tsx`, `frontend/src/web/profiles/RecoveryPanel.tsx`, `frontend/src/web/profiles/ProfileEditor.test.tsx`, `frontend/src/web/App.tsx`, `frontend/vite.config.ts`, `frontend/tests/e2e/server-harness.mjs`, `frontend/tests/e2e/fixtures/app-server.ts`, `frontend/tests/e2e/profile-flow.spec.ts`
...
- **Start-cmd:** `npm --prefix frontend run dev`
- **URL:** `http://127.0.0.1:4311`
- **Files:** `frontend/src/worksheets/find-the-wow/definition.ts`, `frontend/src/worksheets/find-the-wow/generator.ts`, `frontend/src/web/worksheets/find-the-wow/Renderer.tsx`, `frontend/src/worksheets/find-the-wow/generator.test.ts`, `frontend/src/shared/worksheet/registry.ts`, `frontend/src/web/worksheets/registry.ts`, `frontend/src/web/worksheets/registry.test.ts`, `frontend/src/web/generator/GeneratorControls.tsx`, `frontend/tests/e2e/find-the-wow.spec.ts`, `frontend/src/web/App.tsx`
...
- **Done when:** The activity is unavailable without `quantities`; when available, the exact subtype allocations and formulas in Section 4.2 hold; numeral/make quantities stay within `min(countingMax, numeralMax, 20)` and comparisons within `min(countingMax, compareMax, 20)`; insufficient capacity fails before filling; match/compare/complete/draw answers recompute respectively as choice/relation/missing-count/target-count; shared/web registry IDs remain in exact parity; and disabling decorative graphics leaves every learning-essential visual and required response intact. `documentation/educational-basis.md` maps each family and writing mode to the Section 12.5 primary sources, states the ages-4–8/within-20/nonnegative/no-regrouping envelope and non-curriculum/non-assessment limits, and makes no fluency, intervention, placement, or mastery claim. Each per-subtype capacity is counted in the SAME collection its selection draws from, so a shortage fails closed deterministically on every seed rather than by seed lottery; a structural test asserts that invariant, not merely one case. (Carried from Steps 5 and 6, where this exact shape — capacity counted in one unit while selection operated in another — was found twice; a third instance triggers the stop-and-audit halt.)
- **Depends on:** Step 4

### Step 9: Personalization and worksheet options

- **Problem:** Apply the approved worksheet-option contract consistently through the generator UI. Rebuild note (2026-09-07): a prior attempt is preserved, unmerged, on branch `build-step-1788124568` (worktree `C:\Users\abero\dev\worktree_build-step-1788124568`); harvest its verified work via `git diff main...build-step-1788124568` rather than re-deriving it, and land in this rebuild the ONE comprehensive fix from that worktree's `.build-step\audit-stop-and-audit.md`: (1) the options fixture builder takes independent `{counting, numeral, compare}` and `{operand, result}` maxima, defaulting to today's tied values; (2) `bindingMaximumKeys` is routed through all four families as the single "which maximum binds" discriminator; (3) a structural regression test records the binding-key set returned for every (family, subtype) pair across the sweep and asserts every declared arm was observed at least once; (4) at least one sweep fixture per family breaks the `counting == numeral == compare` tie and the `operandMax == resultMax` tie; (5) schema-invalid `0` maxima fixtures become `1`. Do NOT close capacity-guard gaps by adding fixtures one at a time (that produced three instances of one bug-shape and fired stop-and-audit); do not add request logging (`logger: false` is a privacy posture); do not refactor the capacity projection boundary; keep `config/children.example.json` a product artifact, not a test fixture. Round-2 findings and what is already verified are in that worktree's `.build-step\review-*-iter2.md` and the audit's "What is NOT in question" section.
- **Type:** code
